...
TcpClient::TcpClient(const std::string &hostToConnect,
...
void TcpClient::SendRPCMessage(const std::string &message,
                                          std::string &result) {
...
int TcpClient::Connect(const string &ip, const int &port) {
  sockaddr_in address;
  int socket_fd;
...
bool TcpClient::IsIpv4Address(const std::string &ip) {
  struct in_addr addr;
